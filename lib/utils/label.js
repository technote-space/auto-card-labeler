...
const getProjectConfig = (config, project) => {
    ...
    if (misc_1.isRegexpSearchProject()) {
        const key = misc_1.findMatched(Object.keys(config), misc_1.getRegexpSearchProjectFlags(), project);
        if (key) {
            return config[key];
        }
    }
...
const getLabels = (config, project, column) => {
    ...
    if (misc_1.isRegexpSearchColumn()) {
        const key = misc_1.findMatched(Object.keys(projectConfig), misc_1.getRegexpSearchColumnFlags(), column);
        if (key) {
